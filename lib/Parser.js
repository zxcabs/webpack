...
Parser.prototype.walkStatement = function walkStatement(statement) {
	...
	case "ImportDeclaration":
		if(statement.kind === "default") {
			var name = statement.specifiers[0].id.name;
			this.scope.renames["$"+name] = undefined;
			this.scope.definitions.push(name);
			this.applyPluginsBailResult("import default", statement, name, statement.source.value);
		} else if(statement.kind === "named") {
			var mapNameToId = {};
			statement.specifiers.forEach(function(specifier) {
				if(specifier.name) {
					var name = specifier.name.name;
				} else {
					var name = specifier.id.name;
				}
				this.scope.renames["$"+name] = undefined;
				this.scope.definitions.push(name);
				mapNameToId["$"+name]  = specifier.id.name;
			}, this);
			this.applyPluginsBailResult("import named", statement, mapNameToId, statement.source.value);
		} else { // all
			this.applyPluginsBailResult("import", statement, statement.source.value);
		}
		break;
	case "ExportDeclaration":
		if(!statement.declaration && statement.specifiers) {
			if(statement.specifiers.length === 1 && statement.specifiers[0].type === "ExportBatchSpecifier") {
				this.applyPluginsBailResult("export", statement, statement.source && statement.source.value);
			} else {
				var mapNameToId = {};
				statement.specifiers.forEach(function(specifier) {
					if(specifier.name) {
						mapNameToId["$"+specifier.name.name] = specifier.id.name;
					} else {
						mapNameToId["$"+specifier.id.name] = specifier.id.name;
					}
				});
				this.applyPluginsBailResult("export named", statement, mapNameToId, statement.source && statement.source.value);
			}
		} else if(!Array.isArray(statement.declaration) && !statement.specifiers) {
			var decl = statement.declaration;
			if(decl.type === "VariableDeclaration") {
				var name = decl.declarations.map(function(d) {
					return d.id.name;
				});
			} else {
				var name = decl.id.name;
			}
			this.applyPluginsBailResult("export declaration", statement, name);
		} else {
			throw new Error("TODO: " + JSON.stringify(statement));
		}
		break;
	case "ModuleDeclaration":
		if(statement.body) {
			var moduleName = statement.id.value;
			if(!this.applyPluginsBailResult("module", statement, moduleName)) {
			...
			this.scope.renames["$"+name] = undefined;
			this.scope.definitions.push(name);
			this.applyPluginsBailResult("module import", statement, name, statement.source.value);
		}
	}
}
